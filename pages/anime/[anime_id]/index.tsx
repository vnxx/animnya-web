...
export default function AnimePage() {
  ...
  const [currentEpisode, setCurrentEpisode] = useState<AnimeEpisodeType | null>(null)
          ...
          <div className="xl:w-[60%]">
            <div className="space-y-8">
              {(nextEpisode || currentEpisode) && (
                <div className={`grid ${nextEpisode && currentEpisode ? 'grid-cols-2' : 'grid-cols-1'} gap-3`}>
                  {currentEpisode && (
                    <Link href={`/anime/${anime.id}/episode/${currentEpisode.id}`}>
                      <Button className="flex justify-center items-center"><MdOutlineRefresh size='20px' /> <span className="ml-2">Eps {currentEpisode.episode}</span></Button>
                    </Link>
                  )}

                  {nextEpisode && (
                    <Link href={`/anime/${anime.id}/episode/${nextEpisode.id}`}>
                      <Button colorScheme="red" className="flex justify-center items-center"><HiOutlineLightBulb size='20px' /><span className="ml-2">Eps {nextEpisode.episode}</span></Button>
                    </Link>
                  )}
                </div>
              )}
